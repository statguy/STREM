...
cutoff <- 1000
cutoffPopulationSizes <- function(populationSizes, cutoff=Inf) subset(populationSizes, Estimated <= cutoff)
...
plotLogPopulationSize <- function(size, modelName, xlim, ylim, cutoff) {
  library(ggplot2)
  library(scales)  
...
p <- plotLogPopulationSize(subset(logPopulationSizes, modelName==modelNames[4]), subset(logMeanPopulationSizes, modelName==modelNames[4]), modelName=modelNames[4], xlim=xlim, ylim=ylim)
...
p <- plotLogPopulationSize(subset(logPopulationSizes2000, modelName==modelNames[4]), subset(logMeanPopulationSizes2000, modelName==modelNames[4]), modelName=modelNames[4], xlim=xlim, ylim=ylim)
...
p <- plotLogPopulationSize(subset(logPopulationSizes10Days, modelName==modelNames[4]), subset(logMeanPopulationSizes10Days, modelName==modelNames[4]), modelName=modelNames[4], xlim=xlim, ylim=ylim)
...
getPopulationSizesTable <- function(populationSizes, meanPopulationSizes, overEstimate, full=F, file, ...) {
  ...
  x <- ddply(populationSizes, .(Scenario, modelName), function(x, overEstimate) {
    ...
    x <- subset(x, Estimated <= overEstimate)
    ...
    return(data.frame(Intercept=round(coef(y)[1], 0), Slope=round(coef(y)[2], 2), R2=round(summary(y)$r.squared, 2), MeanProp=prop.mean, SDProp=prop.sd, Failed=failed))
  ...
  z <- z[,c("Scenario","Model","Slope","R2","MeanProp","SDProp","Failed")]
...
#i.stat <- function(x) data.frame(o.mean=mean(x$Observed), o.sd=sd(x$Observed), e.mean=mean(x$Estimated), e.sd=sd(x$Estimated),
#                                 pp.mean=round(mean(x$Estimated/x$Observed), 2), pp.sd=round(sd(x$Estimated/x$Observed), 2),
#                                 i.mean=mean(x$intersections), i.sd=sd(x$intersections), ip.mean=round(mean(x$intersections/x$Observed),2), ip.sd=round(sd(x$intersections/x$Observed),2))
#ddply(intersectionsx, .(Scenario, modelName), i.stat)
#ddply(intersections2000x, .(Scenario, modelName), i.stat)
#ddply(intersections10daysx, .(Scenario, modelName), i.stat)

rer.stat <- function(x) data.frame(RERMean=round(mean(x$intersections/x$Observed),2), RERSD=round(sd(x$intersections/x$Observed),2))
i <- ddply(intersectionsx, .(Scenario), rer.stat); i$Scenario=prettyScenario(i$Scenario)
i2000 <- ddply(intersections2000x, .(Scenario), rer.stat); i2000$Scenario=prettyScenario(i2000$Scenario)
i10days <- ddply(intersections10daysx, .(Scenario), rer.stat); i10days$Scenario=prettyScenario(i10days$Scenario)
...
#cip1 <- getCIProportions(scenarios=scenarios, modelNames=modelNames[1:3], panel="a", scenarioNames=scenarios)
#cip2 <- getCIProportions(scenarios=paste0(scenarios, "combined"), modelNames=modelNames[1:3], panel="b", scenarioNames=scenarios)
#cip3 <- getCIProportions(scenarios=paste0(scenarios, "10days"), modelNames=modelNames[1:3], panel="c", scenarioNames=scenarios)
