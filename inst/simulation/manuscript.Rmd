...
getTrueAndEstimatedCounts <- function(scenarios, modelNames, cutoff=c(-Inf,Inf)) {
  ...
  counts <- validation$getMeanCounts(scenarios=scenarios, modelNames=modelNames)
  return(counts)
}

c1 <- getTrueAndEstimatedCounts(scenarios=scenarios, modelNames=modelNames[c(3:5)]); c1$Case <- 1
c2 <- getTrueAndEstimatedCounts(scenarios=paste0(scenarios, "combined"), modelNames=modelNames[c(3:5)]); c1$Case <- 2
c3 <- getTrueAndEstimatedCounts(scenarios=paste0(scenarios, "10days"), modelNames=modelNames[c(3:5)]); c1$Case <- 3
c.all <- rbind(c1, c2, c3)
c.agg <- ddply(c.all, .(iteration), function(x) data.frame(Scenario=x$Scenario[1], Model=x$Model[1], Ratio=mean(x$True/x$Estimated), RatioSD=sd(x$True/x$Estimated)))
...
```{r figure-5}
getCIProportions <- function(scenario, modelName, panel, scenarioNames, cutoff=Inf) {
  study <- getStudy(scenario="A")
  validation <- Validation(study=study, populationSizeCutoff=cutoff)
  a <- validation$getValidatedCredibilityIntervalsProportions(scenarios=scenario, modelNames=modelName)
  b <- validation$getValidatedCredibilityIntervalsProportions(scenarios=scenario, modelNames=modelName, probs=c(.25, .75), probsName="50%")
