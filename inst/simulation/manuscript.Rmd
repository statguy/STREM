...
p <- plotLogPopulationSize(subset(logPopulationSizes, modelName==modelNames[5]), modelName=modelNames[5], "a", xlim=xlim, ylim=ylim)
...
p <- plotLogPopulationSize(subset(logPopulationSizes2000, modelName==modelNames[5]), modelName=modelNames[5], "b", xlim=xlim, ylim=ylim)
...
p <- plotLogPopulationSize(subset(logPopulationSizes10Days, modelName==modelNames[5]), modelName=modelNames[5], "c", xlim=xlim, ylim=ylim)
...
```{r figure-4-init}
getPopulationCorrelations <- function(scenarios, modelNames, cutoff=Inf) {
  study <- getStudy(scenario="A")
...
plotCIProportions <- function(credibilityIntervals) {
  ...
  p <- ggplot(credibilityIntervals, aes(scenario, Proportion, shape=CI, colour=modelName)) +
    ...
    geom_point() +
    scale_shape_manual(values=c(16, 1)) + scale_colour_manual("Model", values=c("black", "darkgray", "gray"), labels=modelLabels) +
    theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent")) +
