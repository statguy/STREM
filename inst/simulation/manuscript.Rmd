...
populationSizes.temporal <- validation$getValidationTemporalPopulationSizes(modelNames=modelNames)

p <- ggplot(populationSizes.temporal, aes(Observed, Estimated)) + geom_point() + stat_smooth(method="lm", colour="black") +
...
```{r figure-4}
validation <- Validation()
populationSizes.spatial <- validation$getValidationSpatialPopulationSizes(modelNames=modelNames)
