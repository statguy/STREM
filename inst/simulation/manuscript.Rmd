...
#stat_smooth(method="lm", se=F, colour="black") +
  ...
  if (!any(abs(size$Estimated) == Inf)) p <- p + stat_smooth(method="lm", se=F, colour="black")
...
logMeanPopulationSizes2000  <- getLogMeanPopulationSizes(populationSizes2000)
...
```{r figure-5-init}
getCIProportions <- function(scenarios, modelNames) {
  study <- getStudy(scenario="A")
  validation <- Validation(study=study, populationSizeOverEstimate=Inf)
...
```{r figure-5abc}
cip1 <- getCIProportions(scenarios=scenarios, modelNames=modelNames)
cip2 <- getCIProportions(scenarios=paste0(scenarios, "combined"), modelNames=modelNames)
cip3 <- getCIProportions(scenarios=paste0(scenarios, "10days"), modelNames=modelNames)
