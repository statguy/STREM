...
prettyModelName <- function(x) {
  lookup <- data.frame(x=modelNames, y=c("ST", "VT", "T", "FMP"), stringsAsFactors=F)
...
p <- ggplot(densitiesScaled) + geom_raster(aes_string(x="x", y="y", fill=yearLayer)) +
  scale_fill_gradientn(colours=terrain.colors(100), na.value="transparent", guide=guide_legend(title=expression(paste("Population density ", (log[10]~individuals/km^2)))), trans="log10", breaks=c(10^-4.5, 10^-4, 10^-3.5, 10^-3, 10^-2.5), labels=c(expression(10^-4.5), expression(10^-4.0), expression(10^-3.5), expression(10^-3.0), expression(10^-2.5)), limits=(10^-4.5, 10^-2.5)) +
...
saveFigure(p, "manuscript-figure-2a.png", width=17, height=8, dpi=200) # draft quality
...
saveFigure(p, "manuscript-figure-2b.png", width=17, height=8, dpi=200) # draft quality
...
plotLogPopulationSize <- function(size, modelName, xlim, ylim) {
  library(ggplot2)
  library(scales)  
  ...
  p <- p + facet_wrap(~Scenario) +
    ...
    theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent"),
                                        ...
                                        plot.margin=unit(c(0, 0, 0, 0), units="line"), #top, right, bottom, and left margins
...
xlim <- log10(c(1,6000)); ylim <- xlim
p <- plotLogPopulationSize(subset(logPopulationSizes, modelName==modelNames[1]), modelName=modelNames[1], xlim=xlim, ylim=ylim)
saveFigure(p, "manuscript-figure-3a1.png")
p <- plotLogPopulationSize(subset(logPopulationSizes, modelName==modelNames[3]), modelName=modelNames[3], xlim=xlim, ylim=ylim)
saveFigure(p, "manuscript-figure-3a2.png")
p <- plotLogPopulationSize(subset(logPopulationSizes, modelName==modelNames[4]), modelName=modelNames[4], xlim=xlim, ylim=ylim)
saveFigure(p, "manuscript-figure-3a3.png")

p <- plotLogPopulationSize(subset(logPopulationSizes2000, modelName==modelNames[1]), modelName=modelNames[1], xlim=xlim, ylim=ylim)
saveFigure(p, "manuscript-figure-3b1.png")
p <- plotLogPopulationSize(subset(logPopulationSizes2000, modelName==modelNames[3]), modelName=modelNames[3], xlim=xlim, ylim=ylim)
saveFigure(p, "manuscript-figure-3b2.png")
p <- plotLogPopulationSize(subset(logPopulationSizes2000, modelName==modelNames[4]), modelName=modelNames[4], xlim=xlim, ylim=ylim)
saveFigure(p, "manuscript-figure-3b3.png")

p <- plotLogPopulationSize(subset(logPopulationSizes10Days, modelName==modelNames[1]), modelName=modelNames[1], xlim=xlim, ylim=ylim)
saveFigure(p, "manuscript-figure-3c1.png")
p <- plotLogPopulationSize(subset(logPopulationSizes10Days, modelName==modelNames[3]), modelName=modelNames[3], xlim=xlim, ylim=ylim)
saveFigure(p, "manuscript-figure-3c2.png")
p <- plotLogPopulationSize(subset(logPopulationSizes10Days, modelName==modelNames[4]), modelName=modelNames[4], xlim=xlim, ylim=ylim)
...
getPopulationSizesTable <- function(populationSizes, meanPopulationSizes, overEstimate, full=F, file, ...) {
  ...
  x <- ddply(populationSizes, .(Scenario, modelName), function(x, overEstimate) {
    failed <- round((1 - nrow(cutoffPopulationSizes(x, overEstimate)) / nrow(x)) * 100)
    ...
    if (any(abs(x$Estimated) == Inf)) return(data.frame(Intercept=NA, Slope=NA, R2=NA, Failed=failed, EstTrue=NA, EstTrueSD=NA, N=NA))
    y <- lm(Estimated~Observed, x)
    prop.mean <- mean(x$Estimated/x$Observed)
    prop.sd <- sd(x$Estimated/x$Observed)
    return(data.frame(Intercept=round(coef(y)[1], 0), Slope=round(coef(y)[2], 2), R2=round(summary(y)$r.squared, 2), EstTrue=prop.mean, EstTrueSD=prop.sd, Failed=failed, N=nrow(x)))
  ...
  z$Scenario <- prettyScenario(z$Scenario) #substr(z$Scenario, 1, 1)
  if (full) return(z)
  
  z <- z[,c("Scenario","Model","Slope","R2","EstTrue","EstTrueSD","Failed","N")]
...
plotPopulationSizesTable <- function(logTable) {
  ...
  p <- ggplot(x, aes(Scenario, value, group=Model)) + geom_point(aes(shape=Model)) + geom_line() +
    facet_grid(variable~Case, scales="free_y") + ylab("") +
...
logTable1$MeanProp <- table1$MeanProp; logTable1$SDProp <- table1$SDProp; logTable1$Failed <- table1$Failed
...
logTable2$MeanProp <- table2$MeanProp; logTable2$SDProp <- table2$SDProp; logTable2$Failed <- table2$Failed
...
logTable3$MeanProp <- table3$MeanProp; logTable3$SDProp <- table3$SDProp; logTable3$Failed <- table3$Failed
...
saveFigure(p, "manuscript-figure-3.png", width=17, height=17, dpi=200)
...
plotIntersectionPopulationSizeTable <- function(ratioTable) {
  ...
  p <- ggplot(x, aes(Scenario, value, group=Case)) + geom_point(aes(shape=Case)) + geom_line() +
    ylab("") + #guides(shape=guide_legend()) +
    theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent"), legend.title=element_text(size=10),
                                        strip.text.y=element_text(size=10), legend.position="bottom",
                                        plot.margin=unit(c(0, 0, -.5, -.5), units="line"),
                                        legend.margin=unit(0, units="line"),
...
saveFigure(p, "manuscript-figure-4x.png")
...
rer.stat <- function(x) data.frame(SizeCount=round(mean(x$intersections/x$Observed),2), SizeCountSD=round(sd(x$intersections/x$Observed),2))
i <- ddply(intersectionsx, .(Scenario), rer.stat); i$Scenario=prettyScenario(i$Scenario); i$Case <- "a"
i2000 <- ddply(intersections2000x, .(Scenario), rer.stat); i2000$Scenario=prettyScenario(i2000$Scenario); i2000$Case <- "b"
i10days <- ddply(intersections10daysx, .(Scenario), rer.stat); i10days$Scenario=prettyScenario(i10days$Scenario); i10days$Case <- "c"
...
getCIProportions <- function(scenarios, modelNames, panel, scenarioNames, cutoff=Inf) {
  study <- getStudy(scenario="A")
  validation <- Validation(study=study, populationSizeOverEstimate=cutoff)
...
plotCIProportions <- function(credibilityIntervals) {
  ...
  p <- ggplot(credibilityIntervals, aes(scenario, Proportion, shape=CI, colour=modelName)) +
    ...
    geom_point() +
    scale_shape_manual(values=c(16, 1)) + scale_colour_manual("Model", values=c("black", "darkgray"), labels=modelLabels) +
    theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent")) +
...
```{r figure-5abc}
cip1 <- getCIProportions(scenarios=scenarios, modelNames=modelNames[1:3], panel="a", scenarioNames=scenarios, cutoff=cutoff)
cip2 <- getCIProportions(scenarios=paste0(scenarios, "combined"), modelNames=modelNames[1:3], panel="b", scenarioNames=scenarios, cutoff=10*cutoff)
cip3 <- getCIProportions(scenarios=paste0(scenarios, "10days"), modelNames=modelNames[1:3], panel="c", scenarioNames=scenarios, cutoff=cutoff)
