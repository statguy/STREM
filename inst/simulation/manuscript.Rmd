...
populationSizes <- getPopulationSizes(scenarios=scenarios, modelNames=modelNames[c(1,3,4)])
populationSizes2000 <- getPopulationSizes(scenarios=paste0(scenarios, "combined"), modelNames=modelNames[c(1,3,4)])
populationSizes10Days <- getPopulationSizes(scenarios=paste0(scenarios, "10days"), modelNames=modelNames[c(1,3,4)])
registerDoMC(cores=detectCores())
...
logPopulationSizesSample <- rbind(
  ...
  subset(logPopulationSizesSample, modelName=="T"),
...
p <- plotLogPopulationSizeSample(subset(logPopulationSizesSample, modelName==c("ST","T","FMP")), modelName=c("ST","T","FMP"), "a", xlim=xlim, ylim=ylim)
saveFigure(p, "manuscript-figure-3sample.png", height=7.87*1.2/2) # draft quality
saveFigure(p, "manuscript-figure-3sample.pdf", height=7.87*1.2/2, dpi=300)
```
...
getPopulationSizesTable <- function(populationSizes, meanPopulationSizes, overEstimate, full=F, file, ...) {
  ...
  x <- ddply(populationSizes, .(Scenario, modelName), function(x, overEstimate) {
    ...
    prop.min <- quantile(log(x$Estimated/x$Observed), .05)
    prop.max <- quantile(log(x$Estimated/x$Observed), .95)
...
rer.stat <- function(x) data.frame(SizeCount=mean(x$intersections/x$Observed),
                                   SizeCountMin=quantile(x$intersections/x$Observed, 0.05),
                                   SizeCountMax=quantile(x$intersections/x$Observed, 0.95))
