...
populationSizeOverEstimate <- 1e100
...
logMeanPopulationSizes <- ddply(populationSizes, .(scenario, modelName), function(x) {
  data.frame(True=mean(x$Observed+1), Estimated=mean(x$Estimated+1), Scenario=x$scenario)
})
...
```{r figure-3b}
plotLogPopulationSize <- function(size, meanSize) {
...
p <- plotLogPopulationSize(subset(logPopulationSizes, modelName==modelNames[1]), subset(logMeanPopulationSizes, modelName==modelNames[1]))
saveFigure(p, "manuscript-figure-3b1.png", width=8, height=4)
p <- plotLogPopulationSize(subset(logPopulationSizes, modelName==modelNames[2]), subset(logMeanPopulationSizes, modelName==modelNames[2]))
saveFigure(p, "manuscript-figure-3b2.png", width=8, height=4)
p <- plotLogPopulationSize(subset(logPopulationSizes, modelName==modelNames[3]), subset(logMeanPopulationSizes, modelName==modelNames[3]))
