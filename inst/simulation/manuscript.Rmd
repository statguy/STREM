...
ddply(intersectionsx, .(Scenario), rer.stat)
ddply(intersections2000x, .(Scenario), rer.stat)
ddply(intersections10daysx, .(Scenario), rer.stat)
#writeTable(subset(logTable1, Model != "VT"), "table-1a.docx", context)
...
plotCIProportions <- function(credibilityIntervals) {
  ...
  p <- ggplot(credibilityIntervals, aes(scenario, Proportion, shape=CI, colour=modelName)) +
    ...
    geom_point() +
    scale_shape_manual(values=c(16, 1)) + scale_colour_manual("Model", values=c("darkgray", "black"), labels=modelLabels) +
