...
p <- ggplot(populationSizes, aes(Observed, Estimated, colour=modelName)) + geom_point() + stat_smooth(method="lm", colour="black") +
  scale_colour_manual("Model", values=c("black", "darkgray"), labels=c("ST", "T")) +
...
p <- ggplot(populationCorrelations, aes(True, Correlation, colour=modelName)) + geom_point() +
  scale_colour_manual("Model", values=c("black", "darkgray"), labels=c("ST", "T")) +
  facet_wrap(~Scenario) + xlab("True population size") +
...
p <- ggplot(credibilityIntervals, aes(scenario, Proportion, shape=Interval, colour=modelName)) + geom_point() +
  scale_shape_manual(values=c(16, 1)) + scale_colour_manual("Model", values=c("black", "darkgray"), labels=c("ST", "T")) +
  theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent")) +
  xlab("Scenario")
print(p)
