...
getPopulationSizesTable <- function(populationSizes, meanPopulationSizes, overEstimate, full=F, file, ...) {
  ...
  x <- ddply(populationSizes, .(Scenario, modelName), function(x, overEstimate) {
    ...
    if (x$modelName == "FMPModel") failed <- NA
...
ddply(intersections, .(Scenario, modelName), i.stat)
ddply(intersections2000, .(Scenario, modelName), i.stat)
ddply(intersections10days, .(Scenario, modelName), i.stat)
