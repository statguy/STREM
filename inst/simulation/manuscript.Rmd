...
getEstimatedDensity = function(scenarios, modelName, iterations, year, sd=FALSE, weightHabitats=F) {
  ddply(data.frame(scenario=scenarios, year=rep(year, length(scenarios)), iteration=iterations, stringsAsFactors=FALSE), .(scenario, iteration), function(x, modelName, sd) {
    ...
    #density <- estimates$getPopulationDensity(habitatWeights=habitatWeights, .parallel=T)
...
#iterations <- as.integer(c(1,4,1,2,5,31))
...
p1 <- ggplot(wdensitiesScaled) +
  ...
  theme(legend.position="bottom", legend.margin=unit(-2, units="line"), legend.title=element_text(size=10), legend.text=element_text(size=10),
        plot.margin=unit(c(-3, 0, 0, -.5), units="line")) #top, right, bottom, and left margins
g1 <- arrangeGrob(p1, main=textGrob("a)", x=0.01, just="left", gp=gpar(fontsize=10)))
saveFigure(g1, "manuscript-figure-2a.png", width=17, height=8.8, dpi=200) # draft quality
saveFigure(g1, "manuscript-figure-2a.pdf", width=17, height=8.8, dpi=300)

p2 <- ggplot(boundaryDF) + geom_path(aes(long, lat, group=group), colour="grey") +
  ...
  theme(panel.grid.major=element_line(colour="grey"),
        plot.margin=unit(c(0, 0, 0, -.5), units="line")) #top, right, bottom, and left margins
g2 <- arrangeGrob(p2, main=textGrob("b)", x=0.01, just="left", gp=gpar(fontsize=10)))
saveFigure(g2, "manuscript-figure-2b.png", width=17, height=6.9, dpi=200) # draft quality
saveFigure(g2, "manuscript-figure-2b.pdf", width=17, height=6.9, dpi=300)

p <- arrangeGrob(g1, g2)
saveFigure(p, "manuscript-figure-2.png", width=17, height=17, dpi=200) # draft quality
saveFigure(p, "manuscript-figure-2.pdf", width=17, height=17, dpi=300)
...
#meanPopulationSizes2000  <- getMeanPopulationSizes(cutoffPopulationSizes(populationSizes2000, 10*cutoff, models=modelNames[c(1,3,5)]))
#logPopulationSizes2000 <- getLogPopulationSizes(cutoffPopulationSizes(populationSizes2000, 10*cutoff, models=modelNames[c(1,3,5)]))
#logMeanPopulationSizes2000  <- getMeanPopulationSizes(logPopulationSizes2000)

#meanPopulationSizes10Days  <- getMeanPopulationSizes(cutoffPopulationSizes(populationSizes10Days, cutoff, models=modelNames[c(1,3,5)]))
#logPopulationSizes10Days <- getLogPopulationSizes(cutoffPopulationSizes(populationSizes10Days, cutoff, models=modelNames[c(1,3,5)]))
...
#x <- subset(populationSizes, modelName=="SmoothModelMean-nbinomial-ar1-priors1" & Scenario=="C")
#y <- lm(Estimated~Observed, x)
...
#table2 <- getPopulationSizesTable(populationSizes2000, meanPopulationSizes2000, overEstimate=10*cutoff, models=modelNames[c(1,3,5)])
#logTable2 <- getPopulationSizesTable(logPopulationSizes2000, logMeanPopulationSizes2000, overEstimate=log10(10*cutoff+1), models=modelNames[1])
#logTable2$Failed <- table2$Failed

#table3 <- getPopulationSizesTable(populationSizes10Days, meanPopulationSizes10Days, overEstimate=cutoff, models=modelNames[c(1,3,5)])
#logTable3 <- getPopulationSizesTable(logPopulationSizes10Days, logMeanPopulationSizes10Days, overEstimate=log10(cutoff+1), models=modelNames[1])
...
plotPopulationSizesTable_DEPRECATED <- function(logTable) {
  library(reshape2)
  facet_labeller <- function(variable, value) {
    measured <- list(EstTrue=expression(alpha), Slope=expression(beta), R2=expression(R^2), Failed="Failed (%)")
    print(paste0(variable,"=",value))
    if (variable=="variable") return(measured[value])
    if (variable=="Case") return(paste("Case", value))
  }

  #x <- melt(logTable, id.vars=c("Scenario", "Model", "Case"), measure.vars=c("EstTrue", "Slope", "R2", "Failed"))
  ...
  p <- ggplot(x, aes(Scenario, value, group=Model)) +
    ...
    geom_point(aes(shape=Model), position=position_dodge(width=.7)) + ylab("") +
    ...
    theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent"), legend.title=element_text(size=10),
                                        ...
                                        plot.margin=unit(c(0, 0.2, 0, -.5), units="line"),  #top, right, bottom, and left margins
                                        legend.margin=unit(-1.5, units="line"))# , legend.key=element_rect(fill="transparent"))
  
  # Expand y-axis 100 for Failed
  x <- data.frame(Scenario="A", Model="FMP", variable="Failed", value=100)
  p <- p + geom_blank(data=x, aes(x=Scenario, y=value, group=Model))
  
  p <- facet_wrap_labeller(p, labels=c(expression(alpha), Slope=expression(beta), R2=expression(R^2), Failed="Failed (%)"))
...
plotPopulationSizesTable <- function(logTable) {
  ...
  p1 <- x %>% filter(variable=="EstTrue") %>%
    ...
    theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent"),
                                        ...
                                        strip.text.x=element_text(size=10), strip.text.y=element_text(size=10),
                                        plot.margin=unit(c(0, 0.2, -.75, 0), units="line")) #top, right, bottom, and left margins
  g1 <- arrangeGrob(p1, main=textGrob("a)", x=0.01, just="left", gp=gpar(fontsize=10)))
  ...
  p2 <- x %>% filter(variable=="Slope") %>%
    ...
    theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent"),
                                        legend.title=element_text(size=10), legend.position="none",
                                        strip.text.x=element_text(size=10), strip.text.y=element_text(size=10),
                                        plot.margin=unit(c(0, 0.2, -.75, 0), units="line"))
  g2 <- arrangeGrob(p2, main=textGrob("b)", x=0.01, just="left", gp=gpar(fontsize=10)))
  ...
  p3 <- x %>% filter(variable=="R2") %>%
    ...
    theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent"),
                                        ...
                                        strip.text.x=element_text(size=10), strip.text.y=element_text(size=10),
                                        plot.margin=unit(c(0, 0.2, -.75, 0), units="line"))
  g3 <- arrangeGrob(p3, main=textGrob("c)", x=0.01, just="left", gp=gpar(fontsize=10)))
  ...
  p4 <- x %>% filter(variable=="Failed") %>%
    ...
    theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent"),
                                        ...
                                        strip.text.x=element_text(size=10), strip.text.y=element_text(size=10),
                                        plot.margin=unit(c(0, 0.2, -.75, 0), units="line"))
  g4 <- arrangeGrob(p4, main=textGrob("d)", x=0.01, just="left", gp=gpar(fontsize=10)))
...
saveFigure(p, "manuscript-figure-3.pdf", height=17, dpi=300)
...
saveFigure(p, "manuscript-figure-3sample.png", height=7.87) # draft quality
saveFigure(p, "manuscript-figure-3sample.pdf", height=7.87, dpi=300)
```
...
plotPopulationCorrelations <- function(populationCorrelations, case) {
  ...
  p <- ggplot(populationCorrelations, aes(True, Correlation)) +
    ...
    theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent"),
                                        plot.title=element_text(size=10),
                                        plot.margin=unit(c(0, 0.2, 0, 0), units="line"),
...
pc2a <- getPopulationCorrelations(scenarios="Acombined", modelNames=modelNames[1], cutoff=10*cutoff)
pc2b <- getPopulationCorrelations(scenarios="Bcombined", modelNames=modelNames[1], cutoff=10*cutoff)
pc2c <- getPopulationCorrelations(scenarios="Ccombined", modelNames=modelNames[1], cutoff=10*cutoff)
pc2d <- getPopulationCorrelations(scenarios="Dcombined", modelNames=modelNames[1], cutoff=10*cutoff)
pc2e <- getPopulationCorrelations(scenarios="Ecombined", modelNames=modelNames[1], cutoff=10*cutoff)
pc2f <- getPopulationCorrelations(scenarios="Fcombined", modelNames=modelNames[1], cutoff=10*cutoff)
pc2 <- rbind(pc2a,pc2b,pc2c,pc2d,pc2e,pc2f)
pc3a <- getPopulationCorrelations(scenarios="A10days", modelNames=modelNames[1], cutoff=cutoff)
pc3b <- getPopulationCorrelations(scenarios="B10days", modelNames=modelNames[1], cutoff=cutoff)
pc3c <- getPopulationCorrelations(scenarios="C10days", modelNames=modelNames[1], cutoff=cutoff)
pc3d <- getPopulationCorrelations(scenarios="D10days", modelNames=modelNames[1], cutoff=cutoff)
pc3e <- getPopulationCorrelations(scenarios="E10days", modelNames=modelNames[1], cutoff=cutoff)
pc3f <- getPopulationCorrelations(scenarios="F10days", modelNames=modelNames[1], cutoff=cutoff)
pc3 <- rbind(pc3a,pc3b,pc3c,pc3d,pc3e,pc3f)
...
p2 <- plotPopulationCorrelations(pc2, "Case 2")
saveFigure(p2, "manuscript-figure-4b.png")
saveFigure(p2, "manuscript-figure-4b.pdf", dpi=300)
p3 <- plotPopulationCorrelations(pc3, "Case 3")
saveFigure(p3, "manuscript-figure-4c.png")
saveFigure(p3, "manuscript-figure-4c.pdf", dpi=300)
```
...
getCIProportions <- function(scenarios, modelNames, panel, scenarioNames, cutoff=Inf) {
  ...
  if ("FMPModel" %in% modelNames) {
    modelNames <- modelNames[!modelNames %in% "FMPModel"]
...
cip1 <- getCIProportions(scenarios=scenarios, modelNames=modelNames[c(1,3,5)], panel="Case 1", scenarioNames=scenarios, cutoff=cutoff)
#cip2 <- getCIProportions(scenarios=paste0(scenarios, "combined"), modelNames=modelNames[c(1,3,5)], panel="Case 2", scenarioNames=scenarios, cutoff=10*cutoff)
#cip3 <- getCIProportions(scenarios=paste0(scenarios, "10days"), modelNames=modelNames[c(1,3,5)], panel="Case 3", scenarioNames=scenarios, cutoff=cutoff)
...
plotCIProportions <- function(credibilityIntervals) {
  ...
  p <- ggplot(credibilityIntervals, aes(scenario, Proportion, shape=Model, colour=CI)) +
    ...
    scale_shape_manual("Model", values=c(17,15,3)) +
    scale_colour_manual("CI", values=c("gray", "black")) +
    #scale_colour_manual("Model", values=c("blue", "green", "red"), labels=modelLabels) +
    theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent")) +
    xlab("Scenario") + ylab("Proportion") + ylim(0, 1) +
    #facet_wrap(~panel) +
    theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(),
          plot.margin=unit(c(0.2, 0.2, 0, 0), units="line"),
          #legend.background=element_rect(fill="transparent"),
...
#cip <- subset(rbind(cip1, cip2, cip3), !(modelName %in% c("SmoothModel-nbinomial-ar1", "SmoothModel-nbinomial-matern-ar1")))
p <- plotCIProportions(cip)
saveFigure(p, "manuscript-figure-5.png", height=7.87/2)
