...
```{r figure-2-test, eval=FALSE}
x <- data.frame(scenario=scenarios[1], iteration=as.integer(1), stringsAsFactors=F)
...
year0 <- year <- 2001
    ...
    studyArea <- FinlandStudyArea$new(context=context)$setup(tolerance=0.001)
    estimates$data <- subset(estimates$data, year == year0)
    density <- estimates$getPopulationDensityInterpolate(getSD=F, maskPolygon=studyArea$boundary)
...
getPopulationSizesTable <- function(populationSizes, meanPopulationSizes, overEstimate, full=F, file) {
  ...
  x <- ddply(populationSizes, .(Scenario, modelName), function(x, overEstimate) {
    ...
    if (any(abs(x$Estimated) == Inf)) return(data.frame(Intercept=NA, Slope=NA, R2=NA, Failed=failed))
    y <- lm(Estimated~Observed, x)
    return(data.frame(Intercept=round(coef(y)[1], 0), Slope=round(coef(y)[2], 2), R2=round(summary(y)$r.squared, 2), Failed=failed))
  ...
  z <- z[,c("Scenario","Model","Slope","R2","EstDev","Failed")]
...
getPopulationSizesTable(logPopulationSizes, logMeanPopulationSizes, overEstimate=log10(2000+1), file=file.path(dir, "table-1a.csv"))
getPopulationSizesTable(logPopulationSizes2000, logMeanPopulationSizes2000, overEstimate=log10(20000+1), file=file.path(dir, "table-1b.csv"))
getPopulationSizesTable(logPopulationSizes10Days, logMeanPopulationSizes10Days, overEstimate=log10(2000+1), file=file.path(dir, "table-1c.csv"))
```
