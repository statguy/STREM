...
```{r figure-3b}
p <- ggplot(logPopulationSizes, aes(Observed, Estimated, colour=modelName)) + geom_point(size=1) +
  stat_smooth(method="lm", aes(colour=modelName), se=F) +
  scale_colour_manual("Model", values=c("lightgray", "darkgray","black"), labels=c("I", "T", "ST")) +
  geom_abline(colour="darkgrey", linetype="longdash") + 
  geom_point(data=logMeanPopulationSizes, aes(True, Estimated, colour=modelName), size=2, shape=15) +
  facet_wrap(~Scenario, scales="free") +
  xlab("True population size") + ylab("Estimated population size") +
  theme_minimal(base_size=10) + theme(panel.border=element_rect(fill="transparent"))
print(p)
saveFigure(p, "manuscript-figure-3b.png", width=8, height=4)
```

```{r figure-3-init-2000}
study <- getStudy(scenario="A") # Get study object, scenario doesn't matter
validation <- Validation(study=study, populationSizeOverEstimate=20000)
...
logPopulationSizes2000$Observed <- log10(logPopulationSizes2000$Observed + 1)
logPopulationSizes2000$Estimated <- log10(logPopulationSizes2000$Estimated + 1)
...
study <- getStudy(scenario="A") # Get study object, scenario doesn't matter
validation <- Validation(study=study, populationSizeOverEstimate=2000)
...
logPopulationSizes10days$Observed <- log10(logPopulationSizes10days$Observed + 1)
logPopulationSizes10days$Estimated <- log10(logPopulationSizes10days$Estimated + 1)
...
prettyModelName <- function(x) {
  lookup <- data.frame(x=c("SmoothModel-nbinomial-matern-ar1", "SmoothModel-nbinomial-ar1", "FMPModel") , y=c("ST", "S", "I"), stringsAsFactors=F)
