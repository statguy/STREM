...
densities <- getEstimatedDensity(scenarios=scenarios, modelName=modelNames[1], iterations=iterations, year=year, sd=F)
...
p <- ggplot(densitiesScaled) + geom_raster(aes_string(x="x", y="y", fill=yearLayer)) + scale_fill_gradientn(colours=terrain.colors(100), na.value="transparent") +
  geom_path(data=boundaryDF, aes(long, lat, group=group), colour="darkgreen") +
  geom_point(data=agents, aes(x, y, colour=Observed), size=1) + scale_color_manual(values=c("darkgray", "black")) + 
  scale_x_continuous(limits=c(limits@xmin, limits@xmax), expand=c(0,0)) +
  scale_y_continuous(limits=c(limits@ymin, limits@ymax), expand=c(0,0)) +
...
getLogMeanPopulationSizes <- function(populationSizes) {  
...
plotLogPopulationSize <- function(size, meanSize, modelName, xlim, ylim) {
  ...
  p <- ggplot(size, aes(Observed, Estimated, colour=modelName)) +
    ...
    #scale_colour_manual("Model", values=c("darkgray","black")) +
    geom_abline(colour="black", linetype="longdash") + 
    #geom_point(data=meanSize, aes(True, Estimated, colour=modelName), size=2, shape=15) +
...
logMeanPopulationSizes  <- getLogMeanPopulationSizes(populationSizes)
...
logMeanPopulationSize2000  <- getLogMeanPopulationSizes(populationSizes2000)
...
logMeanPopulationSizes10Days  <- getLogMeanPopulationSizes(populationSizes10Days)
#validation$summarizePopulationSize(populationSizes10Days)
```

```{r figure-3abc}
p <- plotLogPopulationSize(subset(logPopulationSizes, modelName==modelNames[1]), subset(logMeanPopulationSizes, modelName==modelNames[1]), modelName=modelNames[1], xlim=c(1,10000), ylim=c(1,10000))
saveFigure(p, "manuscript-figure-3a1.png", width=8, height=4)
p <- plotLogPopulationSize(subset(logPopulationSizes, modelName==modelNames[2]), subset(logMeanPopulationSizes, modelName==modelNames[2]), modelName=modelNames[2], xlim=c(1,10000), ylim=c(1,10000))
saveFigure(p, "manuscript-figure-3a2.png", width=8, height=4)
p <- plotLogPopulationSize(subset(logPopulationSizes, modelName==modelNames[3]), subset(logMeanPopulationSizes, modelName==modelNames[3]), modelName=modelNames[3], xlim=c(1,10000), ylim=c(1,10000))
...
p <- plotLogPopulationSize(subset(logPopulationSizes10Days, modelName==modelNames[1]), subset(logMeanPopulationSizes10Days, modelName==modelNames[1]), modelName=modelNames[1], xlim=c(1,10000), ylim=c(1,10000))
saveFigure(p, "manuscript-figure-3c1.png", width=8, height=4)
p <- plotLogPopulationSize(subset(logPopulationSizes10Days, modelName==modelNames[2]), subset(logMeanPopulationSizes10Days, modelName==modelNames[2]), modelName=modelNames[2], xlim=c(1,10000), ylim=c(1,10000))
saveFigure(p, "manuscript-figure-3c2.png", width=8, height=4)
p <- plotLogPopulationSize(subset(logPopulationSizes10Days, modelName==modelNames[3]), subset(logMeanPopulationSizes10Days, modelName==modelNames[3]), modelName=modelNames[3], xlim=c(1,10000), ylim=c(1,10000))
...
getPopulationSizesTable <- function(populationSizes, meanPopulationSizes, full=F, file) {
  ...
  x <- ddply(populationSizes, .(Scenario, modelName), function(x) {
    ...
    data.frame(Intercept=round(coef(y)[1], 0), Slope=round(coef(y)[2], 2), R2=round(summary(y)$r.squared, 2))
  })
...
getPopulationSizesTable(populationSizes, meanPopulationSizes, file=file.path(dir, "table-1a.csv"))
getPopulationSizesTable(populationSizes2000, meanPopulationSizes2000, file=file.path(dir, "table-1b.csv"))
getPopulationSizesTable(populationSizes10Days, meanPopulationSizes10Days, file=file.path(dir, "table-1c.csv"))
```


```{r figure-4-init}
getPopulationCorrelations <- function(modelName) {
  study <- getStudy(scenario="A")
  validation <- Validation(study=study, populationSizeOverEstimate=Inf)
  populationCorrelations <- validation$getValidationSpatialPopulationSizes(modelNames=modelName)
...
```{r figure-4abc}
pc1 <- getPopulationCorrelations(scenarios=scenarios, modelName=modelNames[1])
pc2 <- getPopulationCorrelations(scenarios=paste0(scenarios, "combined"), modelName=modelNames[1])
pc3 <- getPopulationCorrelations(scenarios=paste0(scenarios, "10days"), modelName=modelNames[1])

p1 <- plotPopulationCorrelations(p1)
saveFigure(p1, "manuscript-figure-4a.png", width=4, height=4)
plotPopulationCorrelations(p2)
p2 <- saveFigure(p2, "manuscript-figure-4b.png", width=4, height=4)
plotPopulationCorrelations(p3)
p3 <- saveFigure(p3, "manuscript-figure-4c.png", width=4, height=4)
```
