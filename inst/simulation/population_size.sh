...
if [ "$model" != "FMPModel" ]
then
  python "$exec_path"/parallel_r.py -p 0 -t "$iterations" -n "$max_nodes" -l 20.0 -b ~/tmp/blacklist.txt -v ~/git/Winter-Track-Counts/inst/simulation/validate.R notest "$scenario" "$model"
fi
