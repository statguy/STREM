...
reset
python "$exec_path"/parallel_r.py -t 1:50 -n 60 -l 10.0 -b ~/tmp/blacklist.txt -v ~/git/Winter-Track-Counts/inst/simulation/validate.R notest "$scenario"
reset

#R --vanilla --args notest "$scenario" "$count_days" 1 < ~/git/Winter-Track-Counts/inst/simulation/count_intersections.R
#R --vanilla --args notest "$scenario" 1 < ~/git/Winter-Track-Counts/inst/simulation/estimate.R
