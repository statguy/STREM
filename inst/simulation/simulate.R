...
simulate = function(scenario, iteration, nSurveyRoutes=as.integer(50), nAgents=as.integer(200), nYears=as.integer(20),
  ...
  mss <- {
    ...
    else if (scenario == "E") MovementSimulationScenarioE$new(nAgents=nAgents, years=nYears, days=nDays, CRWCorrelation=CRWCorrelation)$setup(context=context, readHabitatIntoMemory=T)
    else if (scenario == "F") MovementSimulationScenarioF$new(nAgents=as.integer(nAgents/5), years=nYears, days=nDays, CRWCorrelation=CRWCorrelation, BCRWCorrelationBiasTradeoff=BCRWCorrelationBiasTradeoff)$setup(context=context, readHabitatIntoMemory=T)
