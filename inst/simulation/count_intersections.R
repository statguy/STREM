# Run test:
# ./parallel_r.py -t 1:5 -n 2 -l 10.0 -b ~/tmp/blacklist.txt -v ~/git/Winter-Track-Counts/inst/simulation/count_intersections.R test A
# Run full:
...
err <- try({
  countIntersections <- function(scenario, iteration, test) {
    ...
    mss <- if (scenario == "A") MovementSimulationScenarioA$new()$newInstance(context=context)
