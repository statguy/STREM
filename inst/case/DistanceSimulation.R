...
mssIntensive <- MovementSimulationScenarioIntensive$new(nAgents=as.integer(2), nIterations=as.integer(2))$newInstance(context=context)
...
nIterations <- tracks$getNumberOfIterations()
intersections <- NULL
for (i in 1:nIterations) {
  thinnedTracks <- sampleIntervals$getThinnedTracksSampleIntervals(tracks$getTracks(i))
  thinnedTracks$determineDistances()  
  x <- thinnedTracks$findIntersections(surveyRoutes, dimension=1, save=FALSE)  
