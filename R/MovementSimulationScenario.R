...
MovementSimulationScenario <- setRefClass(
  ...
  methods = list(
    ...
    randomizeVector.DEPRECATED = function(locations) {
      library(sp)
      library(raster)
    ...
    randomizeBCRWTrack.DEPRECATED = function(initialLocation, initialAngle, isFirst, nProposal) {
      library(CircStats)
      library(sp)
    ...
    # 2 born = 1 parent survives + 1 offspring/immigration
    # etc.
    randomizeBirthDeath.DEPRECATED = function(param=list(mean=1, sd=1.1)) {
      bdRate <- rlnorm(n=1, meanlog=log(param$mean), sdlog=log(param$sd))
      nTransform <- rpois(length(agents), bdRate)
    ...
    randomizeBCRWTracks.DEPRECATED = function(iteration) {
      library(plyr)
      library(maptools)
      ...
      for (year in 1:years) {
        if (nAgentsCurrent > 0) {
          track <- ldply(1:nAgentsCurrent,
            function(agentIndex, initialLocations, initialAngles, isFirst, nProposal, iteration, nIterations) {
              library(sp)
            ...
            initialLocations=initialLocations, initialAngles=initialAngles, isFirst=isFirst, nProposal=nProposal, iteration=iteration, nIterations=nIterations, .parallel=runParallel)
